...
\begin{frame}{Farbräume}%
  \only<1>{\includegraphics{build/plots/gamut.pdf}}%
  \only<2>{\includegraphics{build/plots/gamut_srgb.pdf}}%
\end{frame}%
...
\begin{frame}[t]{Warum?}
  \begin{tikzpicture}[remember picture, overlay, shift=(current page.center)]
    ...
    \node [xshift=1cm, yshift=-1.5cm, visible on=<2->] at (A.south) {\Large colormap}; 
  \end{tikzpicture}%
\end{frame}
...
\begin{frame}{Spezielle Colormaps für spezielle Daten}%
  \only<1>{\includegraphics{build/plots/worldmap_jet.pdf}}%
  \only<2>{\includegraphics{build/plots/worldmap_divnorm.pdf}}%
\end{frame}%
